    ...
    def forward(self, xs, ts, noise_std, adjoint=False, method="reversible_heun", actions=None):
        ...
        for i in range(ts.shape[0] - 1):
            ...
            if adjoint:
                ...
                if i == 0:
                    cum_log_ratio = log_ratio[-1, :].reshape(1, log_ratio.shape[1])
                else:
                    cum_log_ratio = torch.cat((cum_log_ratio, log_ratio[-1, :].reshape(1, log_ratio.shape[1])), dim=0)
            else:
...
def log_MSE(xs, ts, latent_sde, bm_vis, global_step, train_dir, steps):
    ...
    xs, actions = get_obs_from_initial_state(xs_, xs.size(1), steps=30)
    xs_model = latent_sde.sample_fromx0(x0=xs_, ts=ts, bm=bm_vis, actions=actions, xs=xs, zs=zs).cpu().numpy()
    mse_loss = nn.MSELoss()
    with torch.no_grad():
